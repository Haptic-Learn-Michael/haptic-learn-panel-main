...
export function WavePath({ className, ...props }: WavePathProps) {
	...
	useEffect(() => {
		...
		const observer = new ResizeObserver(() => setPath(progress));
		observer.observe(container.current);
		return () => observer.disconnect();
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, []);

	// Ancho tomado del propio contenedor (no del viewport), para que la curva
	// siempre calce con el ancho real del bloque en el que vive.
